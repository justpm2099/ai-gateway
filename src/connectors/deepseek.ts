...
export class DeepSeekConnector extends BaseConnector {
  constructor(env: Environment) {
    super(env, env.DEEPSEEK_API_KEY, 'https://api.deepseek.com/v1');
  }
...
export async function handleDeepSeek(request: ChatCompletionRequest, env: Environment): Promise<ProviderResponse> {
  const connector = new DeepSeekConnector(env);
