...
export class GrokConnector extends BaseConnector {
  constructor(env: Environment) {
    super(env, env.GROK_API_KEY, 'https://api.x.ai/v1');
  }
...
export async function handleGrok(request: ChatCompletionRequest, env: Environment): Promise<ProviderResponse> {
  const connector = new GrokConnector(env);
